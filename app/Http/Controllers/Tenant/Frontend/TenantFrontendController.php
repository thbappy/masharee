...
class TenantFrontendController extends Controller
{
    ...
    public function homepage()
    {
          ...
          try {
            $pageSlugs = [
                Str::slug('digital-product'),
                Str::slug('about'),
                Str::slug('blog')

            ];

            Page::whereIn('slug',$pageSlugs)->delete();
    ...
    public function sync_product_total(Request $request)
    {
        ...
        $dhlCountry = Country::where('id', $country)->first();
    
        $carts = Cart::instance("default")->content();
        $totalQuantity = $carts->sum('qty');

        $product = Product::with('inventory', 'inventoryDetail','uom')->where('id', 200)->first();
        // return $product;
       ...
       $totalUomQuantity = 0;
        ...
        foreach ($carts as $cartItem) {
            $product = Product::with('uom')->where('id', $cartItem->id)->first();
          
            if ($product && $product->uom) {
                 $totalUomQuantity += $product->uom->quantity;
            }
        }
        ...
        $default_shipping = ShippingMethod::where('is_default', 1)->get();
                ...
                if (get_static_option("dhl_shipping_username") && get_static_option(option_name: "dhl_shipping_password")) {
                ...
                $unitWeight =  $totalUomQuantity ?? 0.5;
                $unitLength = 5; 
                $unitWidth = 5; 
                $unitHeight = 5; 
                
                // Calculate total dimensions based on quantity
                $totalWeight = $unitWeight * $quantity;
                $totalLength = $unitLength; // Assuming they stack in one dimension
                $totalWidth = $unitWidth; // Assuming constant width
                $totalHeight = $unitHeight * $quantity; // Stacking height
                
                // Replace these with actual values
                $accountNumber = '454304068';
                $originCountryCode = 'AE';
                $originCityName = 'Dubai';
                $destinationCountryCode = 'BD';
                $destinationCityName = 'Dhaka';
                ...
                $url = "https://express.api.dhl.com/mydhlapi/test/rates?" . 
                    ...
                    "&originCountryCode=" . urlencode($originCountryCode) . 
                    "&originCityName=" . urlencode($originCityName) . 
                    "&destinationCountryCode=" . urlencode($destinationCountryCode) . 
                    "&destinationCityName=" . urlencode($destinationCityName) . 
                    "&weight=" . urlencode($totalWeight) . 
                    "&length=" . urlencode($totalLength) . 
                    "&width=" . urlencode($totalWidth) . 
                    "&height=" . urlencode($totalHeight) . 
                    "&plannedShippingDate=" . urlencode($plannedShippingDate) . 
                    "&isCustomsDeclarable=" . urlencode($isCustomsDeclarable) . 
                    "&unitOfMeasurement=" . urlencode($unitOfMeasurement);
                
                $headers = [
                    'Message-Reference: ' . Str::random(rand(28, 36)),
                    'Message-Reference-Date: ' . now()->toDateString(),
                    'Plugin-Name: dhl',
                    'Plugin-Version: 2.12.0',
                    'Shipping-System-Platform-Name: Laravel',
                    'Shipping-System-Platform-Version: ' . app()->version(),
                    'Webstore-Platform-Name: Laravel',
                    'Webstore-Platform-Version: ' . app()->version(),
                    'x-version: 2.12.0',
                ...
                \Log::error("DHL API Response: " . $response);
                if ($error) {
                    \Log::error("DHL API Error: " . $error);
                    return response()->json(['error' => $error], 500);
                }
                
                \Log::error("Valid JSON response from DHL API: " . $response);
                
                $decodedResponse = json_decode($response, true);
                
                $interShipping = $decodedResponse['products'] ?? null;
                ...
                //     $username = 'aurasshamimreza';
                //     $password = 'demomail597@gmail.com';
                //     $credentials = base64_encode($username . ':' . $password);
                //     $authorizationHeader = 'Basic ' . $credentials;
        
                //     $originCountryCode = 971;
                //     $originCityName = "Dubai";
        
                //     // $apiUrl = "https://api-mock.dhl.com/mydhlapi/products";
                //     $apiUrl = " CURLOPT_URL => 'https://api-mock.dhl.com/mydhlapi/rates?";
                //     $queryParams = http_build_query([
                //         'accountNumber' => '454304068',
                //         'originCountryCode' => 971,
                //         'originCityName' => 'Dubai',
                //         'destinationCountryCode' => 971,
                ...
                //             'Message-Reference: YOUR_MESSAGE_REFERENCE',
                //             'Message-Reference-Date: ' . now()->toAtomString(),
                //             'Plugin-Name: dhl',
                //             'Plugin-Version: 1.0.0',
                //             'Shipping-System-Platform-Name: Jumlatech',
                //             'Shipping-System-Platform-Version: 1.0.0',
                //             'Webstore-Platform-Name: Jumlatech',
                ...
                //   dd('print response ' . $response);
                
                  }
                
           

        $shipping_tax_markup = themeView('shop.checkout.markup_for_controller.shipping_tax_ajax', compact('shipping_tax', 'interShipping' ,'shipping_methods', 'default_shipping', 'country', 'state', 'city'))->render();
    ...
    public function sync_product_total_wth_shipping_method(Request $request)
    {
        ...
        if($request->shipping_Total != null){
             $shipping_charge = $request->shipping_Total;
        }else{
        ...
        if ($selected_shipping_method?->options?->tax_status == 1) {
            ...
            if($request->shipping_method == 'dhl_express_worldwide' || $request->shipping_method == 'dhl_express_easy' ){
                 $selected_shipping_method = $shipping_charge;
            }else{
