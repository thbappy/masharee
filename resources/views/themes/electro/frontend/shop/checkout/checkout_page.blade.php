    ...
    <script>
        $(function (){
            ...
            $(document).on('click', 'input[name=shipping_method]', function (){
                ...
                let ship_id = $(this).attr('data-id')
                let shippingTotal = $('.shipping_total_' + ship_id ).val();
                $('.shipping-method').val(shipping_method);

                if (total !== undefined)
                {
                    getShippingMethodBasedTotal(shipping_method, $('.coupon-country').val(), $('.coupon-state').val(), total, shippingTotal);
                }
            });

            function getShippingMethodBasedTotal(shipping_method ,country, state, total,shippingTotal) {
                ...
                $.ajax({
                    ...
                    success: (data) => {
                        if (data.type === 'success')
                        {
                            ...
                            if (data.selected_shipping_method?.options?.final_cost !== undefined) {
                            $('.price-shipping span').last().html(currency + data.selected_shipping_method.options.final_cost);
                            } else {
                                $('.price-shipping span').last().html(currency + data.selected_shipping_method);
                            }
                            // $('.price-shipping span').last().html(currency + data.selected_shipping_method);
            ...
            function getDescription(){
                var selectedLi = $('li[data-gateway="manual_payment"]');

                var description = selectedLi.attr('data-description');

                let manual_description = $('.manual_description');

                manual_description.text(description);
            }
            ...
            $(document).on('click', '.payment-gateway-wrapper > ul > li', function (e) {
                ...
                if (gateway === 'manual_payment') {
                    ...
                    $("#trxId").prop('disabled', false);
                    $("#trxId").removeClass('disable-style');
                    manual_description.text($(this).data('description'));
                } else {
                    $("#trxId").prop('disabled', true);
                    $("#trxId").addClass('disable-style');
