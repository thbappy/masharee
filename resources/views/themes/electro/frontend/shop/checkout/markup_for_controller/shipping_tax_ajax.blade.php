...
@if(count($physical_items) > 0)
    ...
    @endif
     ...
     @if($interShipping != null)
        <ul class="coupon-contents-details-list coupon-border">
            @foreach($interShipping ?? [] as $key => $method)
                <li class="coupon-contents-details-list-item" data-country="{{$country}}" data-state="{{$state}}">
                <span class="coupon-radio-item">
                    <input type="radio" id="shipping-option" value="dhl_{{str_replace(' ', '_', strtolower($method['productName']))}}" name="shipping_method" class="shipping_name_{{$key}}"  data-id="{{$key}}">
                <input type="hidden" id="shipping-option"value="{{$method['totalPrice'][0]['price']}}" name="shipping_method_{{ str_replace(' ', '_', strtolower($method['productName'])) }}" class="shipping_total_{{$key}}">
                    <label for="shipping-option-">  
                      DHL-{{$method['productName']}}   
                    </label>  
                </span>
                    <span>{{amount_with_currency_symbol($method['totalPrice'][0]['price'])}}</span>
                </li>
            @endforeach
        </ul>
